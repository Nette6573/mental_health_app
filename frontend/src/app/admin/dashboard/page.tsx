...
function Badge({ children, type = 'blue' }: { children: React.ReactNode; type?: string }) {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 9px', borderRadius: 20, fontSize: 11.5, fontWeight: 600, background: bg, color, border: `1px solid ${border}` }}>
      {children}
    </span>
  )
}
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .sidebar { width: 240px; min-height: 100vh; background: #0d1117; border-right: 1px solid rgba(255,255,255,0.05); position: fixed; top: 0; left: 0; z-index: 50; display: flex; flex-direction: column; transition: transform 0.25s ease; flex-shrink: 0; }
        .sidebar-logo { padding: 20px 18px 16px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; align-items: center; gap: 10px; }
        .logo-img-wrap { width: 36px; height: 36px; border-radius: 10px; overflow: hidden; background: rgba(37,150,190,0.1); display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .nav-section-label { padding: 16px 18px 6px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; color: rgba(139,148,158,0.5); }
        .nav-item { display: flex; align-items: center; gap: 10px; padding: 9px 14px; margin: 1px 8px; border-radius: 10px; cursor: pointer; font-size: 13px; font-weight: 500; color: #8b949e; text-decoration: none; transition: all 0.15s ease; border: 1px solid transparent; width: calc(100% - 16px); text-align: left; background: none; }
        .nav-item:hover { background: rgba(255,255,255,0.04); color: #c9d1d9; }
        .nav-item.active { background: rgba(37,150,190,0.08); color: #2596be; border-color: rgba(37,150,190,0.15); }
        .nav-badge { margin-left: auto; background: #ef4444; color: white; font-size: 10px; font-weight: 700; padding: 1px 6px; border-radius: 20px; min-width: 18px; text-align: center; }
        .sidebar-footer { margin-top: auto; padding: 12px 8px; border-top: 1px solid rgba(255,255,255,0.05); }
        .admin-profile { display: flex; align-items: center; gap: 10px; padding: 10px; border-radius: 10px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.05); }
        .admin-avatar { width: 32px; height: 32px; border-radius: 9px; background: rgba(37,150,190,0.15); border: 1px solid rgba(37,150,190,0.25); display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; color: #2596be; font-family: 'Syne', sans-serif; flex-shrink: 0; }
        ...
        .logout-btn { margin-left: auto; padding: 5px; border-radius: 7px; background: none; border: none; color: #8b949e; cursor: pointer; display: flex; align-items: center; transition: color 0.15s, background 0.15s; }
        .logout-btn:hover { color: #ef4444; background: rgba(239,68,68,0.08); }
        .main { margin-left: 240px; display: flex; flex-direction: column; min-height: 100vh; width: calc(100% - 240px); min-width: 0; overflow-x: hidden; }
        .topbar { height: 56px; background: rgba(13,17,23,0.95); backdrop-filter: blur(12px); border-bottom: 1px solid rgba(255,255,255,0.05); padding: 0 24px; display: flex; align-items: center; gap: 14px; position: sticky; top: 0; z-index: 40; }
        .topbar-title { font-family: 'Syne', sans-serif; font-size: 14px; font-weight: 700; color: #e6edf3; flex: 1; }
        .search-wrap { position: relative; display: flex; align-items: center; }
        .search-icon { position: absolute; left: 10px; color: #8b949e; pointer-events: none; }
        .search-input { background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.07); border-radius: 9px; padding: 7px 14px 7px 32px; font-size: 13px; color: #c9d1d9; outline: none; width: 200px; font-family: 'DM Sans', sans-serif; transition: border-color 0.2s, width 0.2s; }
        .search-input::placeholder { color: #8b949e; }
        .search-input:focus { border-color: rgba(37,150,190,0.4); width: 240px; }
        .live-indicator { display: flex; align-items: center; gap: 5px; font-size: 11px; color: #10b981; font-weight: 600; }
        ...
        .modal-backdrop { position: fixed; inset: 0; z-index: 100; background: rgba(0,0,0,0.7); backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; padding: 16px; animation: fadeIn 0.2s ease; }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }
        .modal-box { background: #1c2330; border: 1px solid rgba(255,255,255,0.08); border-radius: 20px; width: 100%; max-width: 440px; padding: 28px; animation: slideUp 0.25s ease; }
        @keyframes slideUp { from { opacity: 0; transform: translateY(16px); } to { opacity: 1; transform: translateY(0); } }
        .modal-title { font-family: 'Syne', sans-serif; font-size: 17px; font-weight: 800; color: #e6edf3; margin-bottom: 6px; }
        .modal-sub { font-size: 13px; color: #8b949e; margin-bottom: 24px; }
        .settings-row { display: flex; align-items: center; justify-content: space-between; padding: 16px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06); border-radius: 12px; margin-bottom: 12px; }
        ...
        .btn-reset { padding: 8px 16px; border-radius: 9px; font-size: 13px; font-weight: 600; background: rgba(37,150,190,0.1); border: 1px solid rgba(37,150,190,0.3); color: #2596be; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
        .btn-reset:hover { background: rgba(37,150,190,0.2); }
        .btn-reset:disabled { opacity: 0.5; cursor: not-allowed; }
        .btn-close { width: 100%; padding: 11px; background: transparent; border: 1px solid rgba(255,255,255,0.08); border-radius: 10px; color: #8b949e; font-size: 14px; font-weight: 500; cursor: pointer; margin-top: 16px; transition: all 0.15s; }
        .btn-close:hover { border-color: rgba(255,255,255,0.15); color: #c9d1d9; }
        .success-box { padding: 12px 16px; border-radius: 10px; background: rgba(16,185,129,0.08); border: 1px solid rgba(16,185,129,0.2); display: flex; align-items: center; gap: 10px; margin-top: 12px; }
        .error-box { padding: 12px 16px; border-radius: 10px; background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.2); font-size: 13px; color: #f87171; margin-top: 12px; }
      `}</style>
        ...
        <aside className={`sidebar${sidebarOpen ? ' open' : ''}`}>
          <div className="sidebar-logo">
            <div className="logo-img-wrap">
              <img src="/provider-dashboard/images/hopepath.png" alt="HopePath" width={28} height={28} style={{ objectFit: 'cover', borderRadius: 8 }} />
            </div>
            <div>
              ...
              <div className="panels-grid fade-in fade-in-3">
                <div className="panel">
                  <div className="panel-header">
                    ...
                    <button className="action-btn action-btn-blue" onClick={() => router.push('/admin/providers')}>View all</button>
                  </div>
                  {loading ? (
                    <div style={{ padding: 24, textAlign: 'center', color: '#8b949e', fontSize: 13 }}>Loading...</div>
                  ) : providers.length === 0 ? (
                ...
                <div className="panel">
                  <div className="panel-header">
                    ...
                    <button className="action-btn action-btn-blue" onClick={() => router.push('/admin/clients')}>View all</button>
                  </div>
                  {loading ? (
                    <div style={{ padding: 24, textAlign: 'center', color: '#8b949e', fontSize: 13 }}>Loading...</div>
                  ) : users.length === 0 ? (
                        ...
                        <tbody>
                          {users.slice(0, 5).map((u: any) => (
                            <tr key={u.id} style={{ cursor: 'pointer' }} onClick={() => router.push('/admin/clients')}>
                              ...
                              <td style={{ color: '#8b949e', fontSize: 11.5 }}>
                                {u.joinDate ? new Date(u.joinDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}
                              </td>
                            </tr>
                          ))}
                        </tbody>
          ...
          {activePage !== 'dashboard' && (
            <div className="page fade-in">
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 12, color: '#8b949e' }}>
                ...
                <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 17, fontWeight: 700, color: '#e6edf3' }}>
        ...
        <div className="modal-backdrop" onClick={() => { setShowSettingsModal(false); setResetSent(false); setResetError('') }}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
              ...
              <button onClick={() => { setShowSettingsModal(false); setResetSent(false); setResetError('') }} style={{ background: 'none', border: 'none', color: '#8b949e', cursor: 'pointer', padding: 4, borderRadius: 6 }}>
            ...
            <div className="settings-row">
              ...
              <button className="btn-reset" onClick={handleResetPassword} disabled={resetLoading || resetSent}>
            ...
            <button className="btn-close" onClick={() => { setShowSettingsModal(false); setResetSent(false); setResetError('') }}>Close</button>
          </div>
        </div>
      )}
    </>
