...
// ── Cookie helpers (middleware can only read cookies, not localStorage) ────────

function setAdminCookie(expiry: string) {
  document.cookie = `adminToken=mock-token; path=/; expires=${new Date(expiry).toUTCString()}; SameSite=Lax`;
}
...
export function AdminAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleActivity = () => {
      ...
      setAdminCookie(expiry); // keep cookie expiry in sync too
    };
  ...
  const login = async (credentials: AdminLoginCredentials) => {
    ...
    try {
      ...
      // Cookie for middleware, localStorage for client-side rehydration
      setAdminCookie(expiry);
      ...
      throw error; // re-throw so LoginForm can count attempts
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
